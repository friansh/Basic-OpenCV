...
cv2.createTrackbar("l_t", "track", 110, 255, nothing)
cv2.createTrackbar("h_t", "track", 185, 255, nothing)
...
while True:
    ...
    img_lane = cv2.cvtColor(img_lane, cv2.COLOR_BGR2HSV)
    ...
    p2 = [int(round(width*(2/5))), int(round(height*(5/7)))]
    p3 = [int(round(width*(3/5))), int(round(height*(5/7)))]
    ...
    if lines is not None:
        ...
        if l_slope != 0 and l_intercept != 0:
            try:
                y1 = roi.shape[0]
                y2 = int(round(y1 * (5/7)))
                y1_2_left = int(round(y2 + (y1 - y2)/2))
                x1 = int(round((y1 - l_intercept) / l_slope))
                x2 = int(round((y2 - l_intercept) / l_slope))
                x1_2_left = int(round(y1_2_left - l_intercept) / l_slope)

                p1_trap = np.array([x1, y1])
                p2_trap = np.array([x2, y2])

                cv2.line(final_mask, (x1, y1), (x2, y2), (255, 0, 0), 10)
        ...
        if r_slope != 0 and r_intercept != 0:
            try:
                y1 = roi.shape[0]
                y2 = int(round(y1*(5/7)))
                y1_2_right = int(round(y2 + (y1 - y2) / 2))
                x1 = int(round((y1-r_intercept)/r_slope))
                x2 = int(round((y2-r_intercept)/r_slope))
                x1_2_right = int(round(y1_2_right - r_intercept) / r_slope)
        ...
        if len(p1_trap) == 2 and len(p2_trap) == 2 and len(p3_trap) == 2 and len(p4_trap) == 2:
            ...
            cv2.circle(final_mask, (x1_2_right, y1_2_right), 20, (255, 255, 255), -1)
            cv2.circle(final_mask, (x1_2_left, y1_2_left), 20, (255, 255, 255), -1)
    ...
    final_output = cv2.addWeighted(img_lane, 1, final_mask, 0.8, 1)
